...
# labels 에는 답
labels_df = pd.read_csv('../dacon_data/dirty_mnist_2nd_answer.csv')[:]
imgs_dir = np.array(sorted(glob.glob('../dacon_data/dirty_mnist_2nd/*')))[:]
...
#     model = nn.DataParallel(model)
    ...
    #         torch.save(model.state_dict(), '../data/model/EfficientNetB0_1-fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(2):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('../data/model/EfficientNetB0_1-fold{}.pt'.format(fold)))
        model.eval()
...
submission.iloc[:,1:] = np.where(submission.values[:,1:]>=1.3, 1,0)

submission.to_csv('../dacon_data/EfficientNetB0-fold2_1-3.csv', index=False)