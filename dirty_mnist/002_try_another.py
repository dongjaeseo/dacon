...
#     model = nn.DataParallel(model)
    ...
    #         torch.save(model.state_dict(), '../data/model/EfficientNetB4-fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(5):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('../data/model/EfficientNetB4-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('../dacon_data/EfficientNetB4-fold5_2.csv', index=False)